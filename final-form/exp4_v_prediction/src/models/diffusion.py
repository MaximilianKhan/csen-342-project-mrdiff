...
def compute_v_target(
    ...
    """Compute v-prediction target: v = α_t · ε - σ_t · x0.

    V-prediction (Salimans & Ho, 2022) provides uniform gradient variance
    across all timesteps, giving the model a consistent learning signal.
    """
    sqrt_alpha_bar = schedule.get_sqrt_alpha_bar(k)
    sqrt_one_minus_alpha_bar = schedule.get_sqrt_one_minus_alpha_bar(k)
    ...
    return sqrt_alpha_bar * noise - sqrt_one_minus_alpha_bar * y0


def v_to_x0(v_pred, yk, alpha_bar):
    """Convert v-prediction to x0: x0 = α_t · y_t - σ_t · v."""
    alpha = torch.sqrt(alpha_bar)
    sigma = torch.sqrt(1.0 - alpha_bar)
    return alpha * yk - sigma * v_pred


def v_to_eps(v_pred, yk, alpha_bar):
    """Convert v-prediction to epsilon: ε = σ_t · y_t + α_t · v."""
    alpha = torch.sqrt(alpha_bar)
    sigma = torch.sqrt(1.0 - alpha_bar)
    return sigma * yk + alpha * v_pred
