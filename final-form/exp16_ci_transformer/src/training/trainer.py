...
class Trainer:
    ...
    def _validate(self, fast_mode: bool = True) -> Dict[str, float]:
        ...
        for batch in self.val_loader:
            ...
            # Model must be in train mode to compute training_step loss
            # (forward() dispatches to training_step when self.training=True)
            self.model.train()
            ...
            if not fast_mode:
                ...
                total_mae += compute_mae(predictions, forecast).item()
                total_mse += compute_mse(predictions, forecast).item()
