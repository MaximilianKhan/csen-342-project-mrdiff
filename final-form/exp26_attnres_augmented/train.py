#!/usr/bin/env python3
"""Exp 26: CI+Decomp+AttnRes Transformer with Data Augmentation.

Combines Attention Residuals (selective depth-wise retrieval) with temporal
data augmentation (more diverse training signal). AttnRes gives capacity to
exploit augmented diversity; augmentation gives signal for AttnRes to learn from.
"""
...
def augment(lookback, forecast):
    """Gentle temporal augmentations — less aggressive than Exp 22."""
    B, H, D = lookback.shape
    # 1. Light Gaussian jitter (σ=0.01, gentler than Exp 22's 0.03)
    if random.random() < 0.3:
        lookback = lookback + torch.randn_like(lookback) * 0.01
    # 2. Mild scaling (0.95-1.05, gentler than 0.9-1.1)
    if random.random() < 0.3:
        scale = 0.95 + 0.1 * torch.rand(B, 1, 1, device=lookback.device)
        lookback = lookback * scale
        forecast = forecast * scale
    # 3. Window masking: zero out a small random contiguous segment (5-10% of lookback)
    if random.random() < 0.3:
...
def create_model(cfg, D, H, L):
    ...
    return CIAttnResDecompTransformer(
        ...
        patch_size=max(4, ps), d_model=cfg["d_model"],
...
def train_and_eval(model, tl, vl, te, sc, cfg):
    ...
    for epoch in range(100):
        ...
        with torch.no_grad():
            for b in vl:
                vl_sum += nn.functional.mse_loss(
                    model(b["lookback"].to(device)), b["forecast"].to(device)).item()
                vn += 1
        avg_val = vl_sum / vn
    ...
    return torch.cat(all_mae).mean().item(), final_ep

def main():
    print("EXP 26: CI+Decomp+AttnRes + Gentle Augmentation", flush=True)
    with open("configs/small.yaml") as f:
        base = yaml.safe_load(f)

    for exp in EXPERIMENTS:
        tl, vl, te, sc = create_dataloaders(
            data_path=base["data"]["data_path"], dataset_name=exp["dataset"],
        ...
        print(f"  {exp['name']}: MAE={mae:.4f} | {p:,} params | {epochs} ep | {elapsed:.0f}s", flush=True)
