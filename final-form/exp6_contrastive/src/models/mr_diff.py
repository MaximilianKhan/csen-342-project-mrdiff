...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        # Contrastive loss hyperparameters (CCDM)
        self.contrastive_lambda = 0.005  # Weight for InfoNCE loss
        self.contrastive_tau = 0.1       # Temperature for InfoNCE
        self.num_negatives = 4           # Number of negative samples
    ...
    def _generate_negatives(self, target: torch.Tensor) -> List[torch.Tensor]:
        """Generate temporal negative examples for contrastive learning.

        Creates time-shifted and scaled versions of the target that serve
        as "wrong futures" the model should learn to reject.

        Args:
            target: Ground truth target [B, T, D].

        Returns:
            List of negative samples, each [B, T, D].
        """
        B, T, D = target.shape
        negatives = []

        # 1. Time-shifted negatives (roll by random amount)
        for _ in range(self.num_negatives // 2):
            shift = torch.randint(T // 8, T // 2, (1,)).item()
            neg = torch.roll(target, shifts=shift, dims=1)
            negatives.append(neg)

        # 2. Scaled + jittered negatives
        for _ in range(self.num_negatives - self.num_negatives // 2):
            scale = 0.5 + torch.rand(1).item() * 1.5  # Scale between 0.5 and 2.0
            jitter = torch.randn_like(target) * 0.3
            neg = target * scale + jitter
            negatives.append(neg)
    ...
    def _compute_infonce_loss(
        ...
        The positive is the true target's denoising error (should be small).
        Negatives are wrong targets' denoising errors (should be large).
        InfoNCE forces the model to assign high likelihood to the true future.

        Args:
            eps_pred_pos: Epsilon prediction for true target [B, T, D].
            noise: True noise [B, T, D].
            y0_s: True clean component [B, T, D].
            yk_s: Noisy data [B, T, D].
            k: Timestep [B].
            step_embed: Step embedding [B, hidden_dim].
            conditioning: Conditioning signal [B, T, cond_dim].
        ...
        # Positive score: negative MSE of epsilon prediction (higher = better)
        pos_error = F.mse_loss(eps_pred_pos, noise, reduction='none').mean(dim=(1, 2))  # [B]
        pos_score = -pos_error / self.contrastive_tau  # [B]

        # Generate negatives and compute their scores
        negatives = self._generate_negatives(y0_s)
        neg_scores = []

        with torch.no_grad():
            for neg_target in negatives:
                # Noise the negative the same way
                alpha_bar_k = self.schedule.alpha_bars[k].view(-1, 1, 1)
                yk_neg = torch.sqrt(alpha_bar_k) * neg_target + torch.sqrt(1 - alpha_bar_k) * noise

                # Predict epsilon for negative
                eps_pred_neg = self.denoising(
                    stage=stage, y_noisy=yk_neg, step_embed=step_embed,
                    conditioning=conditioning, x0_prev=None,
                )
                neg_error = F.mse_loss(eps_pred_neg, noise, reduction='none').mean(dim=(1, 2))  # [B]
                neg_scores.append(-neg_error / self.contrastive_tau)  # [B]

        # InfoNCE: log(exp(pos) / (exp(pos) + sum(exp(neg))))
        neg_scores = torch.stack(neg_scores, dim=1)  # [B, num_negatives]
        all_scores = torch.cat([pos_score.unsqueeze(1), neg_scores], dim=1)  # [B, 1+num_negatives]

        # Cross entropy with labels=0 (positive is first)
        labels = torch.zeros(B, device=y0_s.device, dtype=torch.long)
    ...
    def training_step(
        ...
        for s in range(self.num_stages - 1, -1, -1):
            ...
            # Contrastive conditioning loss (CCDM) — InfoNCE
            # Only compute for stage 0 (finest) to save compute
            if s == 0:
                infonce_loss = self._compute_infonce_loss(
                    eps_pred, noise, y0_s, yk_s, k, step_embed, conditioning, s,
                )
                loss_s = loss_s + self.contrastive_lambda * infonce_loss
