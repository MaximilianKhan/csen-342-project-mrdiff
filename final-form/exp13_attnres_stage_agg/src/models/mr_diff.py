...
class BackboneConvBlock(nn.Module):
    """Conv1d block: Conv1d -> GroupNorm -> LeakyReLU -> Dropout."""
...
class AttnResPath(nn.Module):
    """Path with Full Attention Residuals (Eq. 1-4 from AttnRes paper)."""

    def __init__(
        self, input_dim, lookback_length, forecast_length,
        num_blocks=4, hidden_channels=64, kernel_size=3, dropout=0.3,
    ):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            BackboneConvBlock(hidden_channels, kernel_size, dropout)
        ...
        self.key_norm = RMSNorm(hidden_channels)
        self.length_proj = nn.Linear(lookback_length, forecast_length)
        self.output_proj = nn.Conv1d(hidden_channels, input_dim, 1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        v0 = self.input_proj(x)
        layer_outputs = [v0]

        for l, block in enumerate(self.blocks):
            sources = torch.stack(layer_outputs, dim=0)
            keys = self.key_norm(sources.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)
            logits = torch.einsum('c, s b c h -> s b h', self.queries[l], keys)
            attn = torch.softmax(logits, dim=0)
            h_l = (attn.unsqueeze(2) * sources).sum(dim=0)
            v_l = block(h_l)
            layer_outputs.append(v_l)

        out = layer_outputs[-1]
        out = self.length_proj(out)
        out = self.output_proj(out)
        return out


class AttnResDeepBackbone(nn.Module):
    """Deep backbone with AttnRes for trend and residual paths."""

    def __init__(
        self, input_dim, lookback_length, forecast_length,
        num_blocks=4, hidden_channels=64, kernel_size=3, dropout=0.3, trend_kernel=25,
    ):
        super().__init__()
        self.trend_kernel = trend_kernel
        self.trend_path = AttnResPath(
            input_dim, lookback_length, forecast_length,
            num_blocks, hidden_channels, kernel_size, dropout,
        )
        self.resid_path = AttnResPath(
            input_dim, lookback_length, forecast_length,
            num_blocks, hidden_channels, kernel_size, dropout,
        )
    ...
    def forward(self, lookback: torch.Tensor) -> torch.Tensor:
        ...
        resid = x - trend
        t = self.trend_path(trend)
        r = self.resid_path(resid)
        return (t + r).transpose(1, 2)


class StageAggregator(nn.Module):
    """Learned softmax aggregation over diffusion stage predictions.

    Instead of equal-weight summation, uses AttnRes-style attention:
      α_s = softmax(w^T · RMSNorm(proj(pred_s)))  over stages
      output = Σ α_s · pred_s

    Zero-initialized query → uniform (equal sum) at start.
    """

    def __init__(self, input_dim: int, forecast_length: int, hidden_dim: int = 64):
        super().__init__()
        self.proj = nn.Linear(input_dim * forecast_length, hidden_dim)
        self.key_norm = RMSNorm(hidden_dim)
        self.query = nn.Parameter(torch.zeros(hidden_dim))  # zero-init

    def forward(self, predictions: List[torch.Tensor]) -> torch.Tensor:
        """predictions: list of [B, T, D] tensors -> [B, T, D]"""
        B = predictions[0].shape[0]
        S = len(predictions)

        # Flatten each prediction and project to key space
        flat = torch.stack([p.reshape(B, -1) for p in predictions], dim=0)  # [S, B, T*D]
        keys = self.key_norm(self.proj(flat))  # [S, B, hidden]

        # Attention logits and weights
        logits = torch.einsum('h, s b h -> s b', self.query, keys)  # [S, B]
        attn = torch.softmax(logits, dim=0)  # [S, B]

        # Weighted sum
        stacked = torch.stack(predictions, dim=0)  # [S, B, T, D]
        result = (attn.unsqueeze(-1).unsqueeze(-1) * stacked).sum(dim=0)  # [B, T, D]
        return result
...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        # Deep direct prediction backbone (Exp 13: AttnRes + Stage Aggregation)
        self.backbone = AttnResDeepBackbone(
            input_dim=input_dim,
            lookback_length=lookback_length,
            forecast_length=forecast_length,
            num_blocks=4,
        ...
        # Learned stage aggregation (replaces equal-weight sum)
        self.stage_aggregator = StageAggregator(
            input_dim=input_dim,
            forecast_length=forecast_length,
            hidden_dim=64,
        )
    ...
    def direct_predict(self, lookback: torch.Tensor) -> torch.Tensor:
        """AttnRes deep backbone forecast from lookback (no diffusion)."""
        return self.backbone(lookback)
    ...
    def sample(
        ...
        for _ in range(num_samples):
            ...
            # Aggregate multi-resolution predictions with learned attention
            if aggregation == "sum":
                forecast = self.stage_aggregator(predictions)
            else:
