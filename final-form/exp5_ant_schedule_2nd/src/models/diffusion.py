...
Modified for ANT (Improvement 4): DiffusionSchedule now accepts
schedule_type='ant' and an ant_betas_path pointing to a pre-computed
.pt file produced by src/data/ant_schedule.py.
"""
...
class DiffusionSchedule:
    ...
    Supports linear, cosine, and ANT (Adaptive Noise for Time series) schedules
    with precomputed α values.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            ant_betas_path: Path to a .pt file containing {'betas': Tensor[K]}
                produced by src/data/ant_schedule.py. Required when
                schedule_type='ant'; ignored otherwise.
        ...
        elif schedule_type == "ant" and ant_betas_path is not None:
            # ANT: load pre-computed dataset-adaptive betas from disk.
            # The file is produced by src/data/ant_schedule.py.
            loaded = torch.load(ant_betas_path, map_location="cpu")
            if "betas" not in loaded:
                raise KeyError(
                    f"ANT betas file '{ant_betas_path}' is missing the 'betas' key. "
                    "Re-run src/data/ant_schedule.py to regenerate it."
                )
            betas = loaded["betas"].float()
            if betas.shape[0] != num_steps:
                raise ValueError(
                    f"ANT betas file has {betas.shape[0]} steps but model "
                    f"diffusion_steps={num_steps}. Re-run ant_schedule.py with "
                    f"--num-steps {num_steps} to regenerate the file."
                )
            self.betas = betas.to(self.device)

        else:
            if schedule_type == "ant" and ant_betas_path is None:
                # Graceful fallback: warn but don't crash.
                # This can happen if the user forgot to run the pre-compute step.
                import warnings
                warnings.warn(
                    "schedule_type='ant' but ant_betas_path is None. "
                    "Falling back to linear schedule. Run src/data/ant_schedule.py "
                    "to generate the betas file, then set ant_betas_path in the config.",
                    RuntimeWarning,
                    stacklevel=2,
                )
