"""Channel-Independent Decomposed Transformer (Exp 17).
...
class CIDecompTransformer(nn.Module):
    ...
    def _encode_and_project(self, x_1d: torch.Tensor, temporal_proj, channel_proj, B: int, D: int) -> torch.Tensor:
        ...
        # Embed + position: [B*D, N, d_model]
        x = self.patch_embed(x) + self.pos_embed

        # Transformer: [B*D, N, d_model]
        x = self.transformer(x)
        x = self.head_norm(x)

        # Temporal projection: [B*D, d_model, N] → [B*D, d_model, T]
