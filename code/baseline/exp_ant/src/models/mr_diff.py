...
Modified for ANT (Improvement 4): MRDiff.__init__ and create_model() now
accept ant_betas_path and forward it to DiffusionSchedule.

The ant_betas_path supports a {dataset} placeholder so that a single config
file works for all four experiment variants (ETTh1/ETTm1 × multi/uni).
The placeholder is resolved at create_model() time using the dataset name
already in the config (which run_experiments.py overrides per experiment).
"""
...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        ant_betas_path: Optional[str] = None,
        ...
        Args:
            ...
            ant_betas_path: Path to pre-computed ANT betas .pt file.
                Required when schedule_type='ant'. Produced by
                src/data/ant_schedule.py. {dataset} in the path has already
                been resolved by create_model() before this is called.
        ...
        # Diffusion schedule — ANT path resolved before being passed in
        self.schedule = DiffusionSchedule(
            num_steps=diffusion_steps,
            beta_start=beta_start,
            beta_end=beta_end,
            schedule_type=schedule_type,
            ant_betas_path=ant_betas_path,
        )
...
def create_model(config: dict) -> MRDiff:
    ...
    ANT path resolution: if ant_betas_path contains the literal string
    '{dataset}', it is replaced with the actual dataset name from
    config['data']['dataset'] at this point. This allows a single config
    file to serve all four experiment variants without manual edits.

    Example:
        ant_betas_path: 'data/ant_betas_{dataset}.pt'
        dataset: ETTm1
        → resolved to: 'data/ant_betas_ETTm1.pt'
    ...
    # Resolve ANT betas path template
    ant_betas_path = training_config.get("ant_betas_path", None)
    if ant_betas_path is not None and "{dataset}" in ant_betas_path:
        dataset_name = data_config.get("dataset", "ETTh1")
        ant_betas_path = ant_betas_path.replace("{dataset}", dataset_name)
