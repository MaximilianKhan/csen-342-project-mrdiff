...
"""
import torch, torch.nn as nn, yaml, time, random, copy
...
def augment(lookback, forecast):
    """Gentle augmentation (same as Exp 26)."""
...
def create_model(cfg, D, H, L):
    ...
    stride = cfg.get("patch_stride", None)  # None -> non-overlapping (original behaviour)
    if cfg["arch"] == "attnres":
...
def train_model(model, train_loader, val_loader, cfg):
    """Train a single model. Returns best state dict."""
