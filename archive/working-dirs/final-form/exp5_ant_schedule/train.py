...
Computes IAAT from training data and uses it to create a dataset-specific
beta schedule that accounts for temporal non-stationarity.
"""
...
from src.models.diffusion import compute_iaat
...
def main():
    print("=" * 70)
    print("EXPERIMENT 5: ANT Adaptive Noise Schedule")
    print("=" * 70)

    with open("configs/small.yaml") as f:
        base_config = yaml.safe_load(f)

    # Override schedule type to ANT
    base_config["training"]["schedule_type"] = "ant"
    ...
    for exp in EXPERIMENTS:
        ...
        # Compute IAAT from training data
        train_data = train_loader.dataset.split_data
        iaat = compute_iaat(train_data, max_lag=50)
        print(f"  IAAT: {iaat:.2f}")

        # Create model with ANT schedule
        model = create_model(config, iaat=iaat)
        params = sum(p.numel() for p in model.parameters())
        print(f"  Params: {params:,}")

        # Print beta schedule info
        betas = model.schedule.betas
        print(f"  Beta range: [{betas[0]:.6f}, {betas[-1]:.6f}]")
        print(f"  Beta median: {betas[len(betas)//2]:.6f}")
    ...
    # Summary
    print(f"\n{'='*70}")
    print("EXPERIMENT 5 RESULTS — ANT Adaptive Noise Schedule")
    print(f"{'='*70}")
    print(f"{'Experiment':<18} {'Direct MAE':>11} {'Full MAE':>11} {'Full MSE':>11} {'IAAT':>8} {'Epochs':>8} {'Time':>8}")
    print("-" * 80)
    for name, r in results.items():
        print(f"{name:<18} {r['mae_direct']:>11.4f} {r['mae_full']:>11.4f} "
              f"{r['mse_full']:>11.4f} {r['iaat']:>8.1f} {r['final_epoch']:>8} {r['train_time_min']:>7.1f}m")
