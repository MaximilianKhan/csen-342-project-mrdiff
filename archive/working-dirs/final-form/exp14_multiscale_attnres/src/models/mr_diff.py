...
class RMSNorm1d(nn.Module):
    """RMSNorm over the channel dimension for [B, D, T] tensors."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: [B, D, T] — norm over D (dim=1)
        rms = torch.sqrt(x.pow(2).mean(dim=1, keepdim=True) + self.eps)
        return self.scale.unsqueeze(0).unsqueeze(-1) * x / rms


class MultiScaleAttnResDLinear(nn.Module):
    """Multi-Scale DLinear with AttnRes fusion (Exp 14).

    N parallel DLinear projections at different temporal scales (kernel sizes),
    fused with AttnRes-style learned softmax attention. Width, not depth.

    Each scale: avg_pool trend extraction → separate trend/resid linear projections.
    AttnRes query (zero-init) learns which scale matters most per dataset.
    """
    ...
    def __init__(
        ...
        lookback_length: int,
        forecast_length: int,
        kernel_sizes: tuple = (5, 15, 25, 51),
    ):
        super().__init__()
        self.kernel_sizes = kernel_sizes
        self.n_scales = len(kernel_sizes)

        # Per-scale DLinear projections (each is lightweight)
        self.trend_projs = nn.ModuleList([
            nn.Linear(lookback_length, forecast_length)
        ...
        # AttnRes fusion: learned query + RMSNorm on keys
        # Query is per-output-timestep to allow time-varying scale selection
        self.attn_query = nn.Parameter(torch.zeros(input_dim))  # [D], zero-init
        self.key_norm = RMSNorm1d(input_dim)

    def _dlinear_at_scale(self, x: torch.Tensor, kernel_size: int, idx: int) -> torch.Tensor:
        """Run one DLinear at a specific trend-extraction scale.

        Args:
            x: [B, D, H]
            kernel_size: avg-pool kernel for trend extraction
            idx: index into projection modules
        ...
        trend = nn.functional.avg_pool1d(x_padded, kernel_size, stride=1)
        resid = x - trend
        return self.trend_projs[idx](trend) + self.resid_projs[idx](resid)

    def forward(self, lookback: torch.Tensor) -> torch.Tensor:
        """lookback: [B, H, D] -> forecast: [B, T, D]"""
        x = lookback.transpose(1, 2)  # [B, D, H]

        # Run all scales in parallel
        scale_outputs = []
        for i, ks in enumerate(self.kernel_sizes):
            scale_outputs.append(self._dlinear_at_scale(x, ks, i))  # [B, D, T]

        # Stack: [N_scales, B, D, T]
        stacked = torch.stack(scale_outputs, dim=0)

        # AttnRes fusion
        # Keys: RMSNorm each scale's output over D dimension
        keys = self.key_norm(stacked)  # [N_scales, B, D, T]

        # Logits: query^T · key, dot product over D → [N_scales, B, T]
        logits = torch.einsum('d, s b d t -> s b t', self.attn_query, keys)

        # Softmax over scales
        attn = torch.softmax(logits, dim=0)  # [N_scales, B, T]

        # Weighted sum: [N_scales, B, D, T] * [N_scales, B, 1, T] → [B, D, T]
        fused = (attn.unsqueeze(2) * stacked).sum(dim=0)

        return fused.transpose(1, 2)  # [B, T, D]
...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        # Multi-scale AttnRes DLinear backbone (Exp 14)
        self.backbone = MultiScaleAttnResDLinear(
            input_dim=input_dim,
            lookback_length=lookback_length,
            forecast_length=forecast_length,
            kernel_sizes=(5, 15, 25, 51),
        )
    ...
    def direct_predict(self, lookback: torch.Tensor) -> torch.Tensor:
        """Multi-scale AttnRes DLinear forecast (no diffusion)."""
