...
class RMSNorm(nn.Module):
    """RMSNorm for AttnRes keys (Eq. 2-3 in Attention Residuals paper)."""
...
class AttnResPath(nn.Module):
    """A single path (trend or residual) with Full Attention Residuals.

    Implements Eq. 1-4 from the Attention Residuals paper:
      h_l = Σ_{i=0}^{l-1} α_{i→l} · v_i
      α_{i→l} = softmax_over_i( w_l^T · RMSNorm(v_i) )

    where w_l is a learned pseudo-query per layer (zero-initialized for
    uniform attention at start = equivalent to standard residuals).
    """
    ...
    def __init__(
        ...
        # Input projection: D -> hidden_channels
        self.input_proj = nn.Conv1d(input_dim, hidden_channels, 1)

        # Conv blocks (transformations f_l)
        self.blocks = nn.ModuleList([
            BackboneConvBlock(hidden_channels, kernel_size, dropout)
            for _ in range(num_blocks)
        ])

        # AttnRes: learned pseudo-query per layer, zero-init → uniform attention at start
        self.queries = nn.ParameterList([
            nn.Parameter(torch.zeros(hidden_channels))
            for _ in range(num_blocks)
        ])

        # Shared RMSNorm for keys (critical per ablation: Table 4 in paper)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """x: [B, C_in, H] -> [B, D, T]"""
        v0 = self.input_proj(x)  # [B, C, H]
        layer_outputs = [v0]

        for l, block in enumerate(self.blocks):
            # Stack all prior outputs: [num_sources, B, C, H]
            sources = torch.stack(layer_outputs, dim=0)

            # RMSNorm keys over C dimension
            # [num_sources, B, C, H] -> permute -> norm over C -> permute back
            keys = self.key_norm(sources.permute(0, 1, 3, 2)).permute(0, 1, 3, 2)

            # Attention logits: w_l^T · k_i, dot product over C
            logits = torch.einsum('c, s b c h -> s b h', self.queries[l], keys)

            # Softmax over depth (sources)
            attn = torch.softmax(logits, dim=0)  # [num_sources, B, H]

            # Weighted sum: h_l = Σ α · v
            h_l = (attn.unsqueeze(2) * sources).sum(dim=0)  # [B, C, H]

            # Apply transformation f_l
            v_l = block(h_l)
            layer_outputs.append(v_l)

        out = layer_outputs[-1]
        out = self.length_proj(out)  # [B, C, T]
        out = self.output_proj(out)  # [B, D, T]
        return out


class AttnResDeepBackbone(nn.Module):
    """Deep backbone with Attention Residuals for both trend and residual paths."""
    ...
    def forward(self, lookback: torch.Tensor) -> torch.Tensor:
        ...
        return (t + r).transpose(1, 2)  # [B, T, D]
...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        # Deep direct prediction backbone (Exp 12: Attention Residuals)
