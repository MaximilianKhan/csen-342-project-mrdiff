#!/usr/bin/env python3
"""Exp 25: CI+Decomp Transformer + Frequency-Enhanced Dual Branch."""
import torch, torch.nn as nn, yaml, time
from src.data.dataset import create_dataloaders
from src.models.ci_freq_transformer import CIFreqTransformer
...
def create_model(cfg, D, H, L):
    ps = cfg["patch_size"]
    while L // ps < 4: ps //= 2
    return CIFreqTransformer(input_dim=D, forecast_length=H, lookback_length=L,
...
def train_and_eval(model, tl, vl, te, sc, cfg):
    ...
    for epoch in range(100):
        model.train()
        for b in tl:
            loss = nn.functional.mse_loss(model(b["lookback"].to(device)), b["forecast"].to(device))
...
def main():
    print("EXP 25: CI+Decomp + Frequency-Enhanced Branch")
