...
class BackboneConvBlock(nn.Module):
    """Conv1d block for the deep backbone: Conv1d -> GroupNorm -> LeakyReLU -> Dropout."""
...
class DeepBackbone(nn.Module):
    """Deep DLinear backbone with Conv1d blocks and standard additive residuals.

    Replaces the flat linear projections with a stack of conv blocks that
    capture local nonlinear temporal patterns before projecting to forecast length.
    """
    ...
    def __init__(
        ...
        # Trend path: D -> hidden -> N blocks -> hidden -> project to T
        self.trend_in = nn.Conv1d(input_dim, hidden_channels, 1)
        ...
        self.trend_out = nn.Conv1d(hidden_channels, input_dim, 1)

        # Residual path: same structure
        self.resid_in = nn.Conv1d(input_dim, hidden_channels, 1)
        self.resid_blocks = nn.ModuleList([
            BackboneConvBlock(hidden_channels, kernel_size, dropout)
            for _ in range(num_blocks)
        ])
        self.resid_proj = nn.Linear(lookback_length, forecast_length)
        self.resid_out = nn.Conv1d(hidden_channels, input_dim, 1)
    ...
    def forward(self, lookback: torch.Tensor) -> torch.Tensor:
        ...
        # Trend path with standard additive residuals
        t = self.trend_in(trend)  # [B, C, H]
        for block in self.trend_blocks:
            t = t + block(t)  # Standard additive residual
        t = self.trend_proj(t)  # [B, C, T]
        t = self.trend_out(t)   # [B, D, T]

        # Residual path with standard additive residuals
        r = self.resid_in(resid)  # [B, C, H]
        for block in self.resid_blocks:
            r = r + block(r)  # Standard additive residual
        r = self.resid_proj(r)  # [B, C, T]
        r = self.resid_out(r)   # [B, D, T]
...
class MRDiff(nn.Module):
    ...
    def __init__(
        ...
        # Deep direct prediction backbone (Exp 11: standard residual connections)
        self.backbone = DeepBackbone(
            input_dim=input_dim,
            lookback_length=lookback_length,
            forecast_length=forecast_length,
            num_blocks=4,
    ...
    def direct_predict(self, lookback: torch.Tensor) -> torch.Tensor:
        """Deep backbone forecast from lookback (no diffusion).
