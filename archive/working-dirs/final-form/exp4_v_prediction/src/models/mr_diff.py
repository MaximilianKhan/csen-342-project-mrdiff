...
from .diffusion import DiffusionSchedule, DiffusionStepEmbedding, forward_diffusion, compute_v_target, v_to_x0
...
class MRDiff(nn.Module):
    ...
    def training_step(
        ...
        for s in range(self.num_stages - 1, -1, -1):
            ...
            # Compute v-target for this step
            v_target = compute_v_target(y0_s, noise, k, self.schedule)

            # Self-conditioning: 50% of the time, get a preliminary x0 estimate first
            x0_prev = None
            if torch.rand(1).item() < 0.5:
                with torch.no_grad():
                    v_prelim = self.denoising(
                    ...
                    x0_prev = v_to_x0(v_prelim, yk_s, alpha_bar_k)
                    x0_prev = x0_prev.detach()

            # Predict v (v-prediction parameterization) with self-conditioning
            v_pred = self.denoising(
                stage=s,
                y_noisy=yk_s,
            ...
            # Recover x0 from v-prediction for scheduled sampling
            alpha_bar_k = self.schedule.alpha_bars[k].view(-1, 1, 1)
            x0_pred = v_to_x0(v_pred, yk_s, alpha_bar_k)
            x0_pred = x0_pred.clamp(-10, 10)  # Prevent explosion at high noise levels
            stage_predictions[s] = x0_pred.detach()

            # Compute loss: v-prediction (uniform gradient variance across k)
            loss_s = nn.functional.mse_loss(v_pred, v_target)
    ...
    def _sample_stage_ddpm(self, s, lookback, coarse_trend, batch_size, device,
                           epsilon_scale=1.0):
        """DDPM 100-step reverse diffusion for a single stage (v-prediction)."""
        yk = torch.randn(
        ...
        for k in range(self.diffusion_steps - 1, -1, -1):
            ...
            # Model predicts v with self-conditioning
            v_pred = self.denoising(
            ...
            x0_pred = v_to_x0(v_pred, yk, alpha_bar)
    ...
    def _sample_stage_dpm(self, s, lookback, coarse_trend, batch_size, device,
        ...
        """DPM-Solver++ sampling for a single stage (v-prediction)."""
