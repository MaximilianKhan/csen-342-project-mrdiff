...
# ──────────────────────────────────────────────────────────────────────
# Usage Example: Integrating with mr-Diff
# ──────────────────────────────────────────────────────────────────────

def example_usage():
    """Demonstrates how DPM-Solver++ replaces DDPM in mr-Diff sampling.

    Before (DDPM, 100 steps):
        for k in range(99, -1, -1):
            x0_pred = model(yk, k)
            yk = ddpm_update(yk, x0_pred, k)  # 100 neural network calls

    After (DPM-Solver++, 20 steps):
        solver = DPMSolverPP(schedule, num_solver_steps=20)
        x0 = solver.sample(model_fn, shape, device)  # 20 neural network calls
    """
    from src.models.diffusion import DiffusionSchedule

    # Initialize schedule (same as training)
    schedule = DiffusionSchedule(
        num_steps=100, beta_start=1e-4, beta_end=0.1
    )

    # Create solver (20 steps instead of 100)
    solver = DPMSolverPP(schedule, num_solver_steps=20)

    print("DPM-Solver++ Configuration:")
    print(f"  Diffusion steps (K):  {schedule.num_steps}")
    print(f"  Solver steps (M):     {solver.num_solver_steps}")
    print(f"  Speedup:              {schedule.num_steps / solver.num_solver_steps:.1f}x")
    print(f"  Timestep schedule:    {solver.timesteps[:6]}...{solver.timesteps[-3:]}")
    print(f"  Log-SNR range:        [{solver.log_snr[0]:.2f}, {solver.log_snr[-1]:.2f}]")

    # In the actual model, sample() would be called per-stage:
    #
    #   for s in range(num_stages - 1, -1, -1):
    #       def model_fn(yk, k):
    #           step_embed = self.step_embedding(k)
    #           cond = self.conditioning(stage=s, history=lookback, ...)
    #           return self.denoising(stage=s, y_noisy=yk, ...)
    #
    #       predictions[s] = solver.sample(model_fn, shape, device)
