...
Diffusion is the wrong tool here, and our results show why. It is data-hungry, but ETT is tiny. Its multi-step sampling accumulates exposure-bias error that drifts predictions toward zero on small data. And it brings a stochastic generator to a target that is essentially deterministic trend and daily seasonality, scored by a single point estimate, so it models uncertainty the task neither contains nor rewards. The inductive biases that actually matter are trend/residual decomposition and channel independence, not denoising. We replaced the 843K-parameter pipeline with a Channel-Independent Decomposed Patch Transformer: no diffusion, a single forward pass, minutes to train. Its parameter count is a range, 54 to 182K, because we tune it per benchmark, and channel independence holds that count fixed no matter how many variables a dataset has. The result beats our own baseline on 3 of 4 benchmarks and the published paper on ETTh1 univariate by 27%, with further gains from a hyperparameter sweep, a cross-variate iTransformer ensemble, and a daily-cycle two-scale decomposition.
