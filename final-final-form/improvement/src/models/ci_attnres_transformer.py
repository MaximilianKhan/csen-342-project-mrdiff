"""CI+Decomp Transformer with Attention Residuals.

Each transformer layer selectively attends over ALL prior layer outputs
via a learned pseudo-query, replacing standard residual connections.
"""
...
class AttnResTransformerLayer(nn.Module):
    """Transformer layer where the residual connection is replaced by
    learned attention over all prior layer outputs (AttnRes)."""
    ...
    def __init__(self, d_model, nhead, dim_feedforward, dropout):
        ...
        # Zero-init -> uniform attention at start = standard residual
        self.attn_query = nn.Parameter(torch.zeros(d_model))
        self.key_norm = RMSNorm(d_model)

    def _attnres_aggregate(self, layer_outputs):
        """Weighted combination of all prior layer outputs. [list of [B,N,d]] -> [B,N,d]"""
        sources = torch.stack(layer_outputs, dim=0)  # [S, B, N, d]
...
class CIAttnResDecompTransformer(nn.Module):
    ...
    Pipeline: lookback -> trend/residual decomp -> CI flatten -> patch embed
    -> AttnRes transformer -> dual output heads -> sum components.
    """
