...
For ETTm1, uses two-scale decomposition (k_fine=15, k_coarse=96 for ETTm1).
...
BEST_CONFIGS = [
    ...
    {"name": "ETTm1_multi", "dataset": "ETTm1", "uni": False, "L": 1440, "H": 192,
     ...
     "dropout": 0.3, "trend_kernel_fine": 15, "trend_kernel_coarse": 96,
     "lr": 0.001, "wd": 0.01},
    {"name": "ETTm1_uni", "dataset": "ETTm1", "uni": True, "L": 1440, "H": 192,
     "arch": "twoscale", "augment": False,
     "patch_size": 16, "d_model": 32, "num_layers": 3, "dim_feedforward": 128,
     "dropout": 0.2, "trend_kernel_fine": 25, "trend_kernel_coarse": 96,
     "lr": 0.0005, "wd": 0.05},
]
...
def main():
    print("Best Single Model Per Benchmark (Exp 31 — Two-Scale Decomposition)", flush=True)
    print("=" * 60, flush=True)
